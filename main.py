...
def main():
    ...
    if k > 0:
        if args.algo == 'backprop':
            kfold.eval(k, args.dataset, initialization_method, backprop.train, learning_rate=learning_rate, batch_size=batch_size, n_epochs=n_epochs)
        elif args.algo == 'astar':
            kfold.eval(k, args.dataset, initialization_method, a_star.train, goal=goal, batch_size=batch_size, should_plot_accuracy_vs_depth=False)
    else:
        ...
        if args.algo == 'backprop':
            backprop.train(neural_network, train_inputs, train_labels, test_inputs, test_labels, learning_rate=learning_rate, batch_size=batch_size, n_epochs=n_epochs, dataset=args.dataset)
        elif args.algo == 'astar':
            a_star.train(neural_network, train_inputs, train_labels, test_inputs, test_labels, goal=goal, batch_size=batch_size, dataset=args.dataset, should_plot_accuracy_vs_depth=True)
